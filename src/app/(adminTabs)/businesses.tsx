...
export default function BusinessesScreen() {
  ...
  function handleSaveBusiness() {
    ...
    if (!businessTypeId) { Alert.alert("Error", "Please select a business type."); return; }
    if (!ownerId) { Alert.alert("Error", "Please select a business owner."); return; }

    const regNumber = editingBusiness ? editingBusiness.registration_number : generateRegNumber();
    let result;
    if (editingBusiness) {
      result = updateBusiness(editingBusiness.id, businessName, regNumber, businessTypeId, ownerId, address || null, businessPhone || null, businessEmail || null, null, isActive ? 1 : 0);
    } else {
      result = addBusiness(businessName, regNumber, businessTypeId, ownerId, address || null, businessPhone || null, businessEmail || null, null);
    }
    if (result.success) { closeBusinessModal(); loadAllData(); }
    Alert.alert(result.success ? "Success" : "Error", result.message);
  }

  function handleDeleteBusiness(id: number) {
    Alert.alert("Confirm Delete", "Delete this business?", [
      { text: "Cancel", style: "cancel" },
      { text: "Delete", style: "destructive", onPress: () => { const result = deleteBusiness(id); if (result.success) loadAllData(); Alert.alert(result.success ? "Success" : "Error", result.message); } },
    ]);
  }
  ...
  function openOwnerModal(owner?: BusinessOwner) {
    if (owner) {
      ...
      setOwnerName(""); setOwnerNationalId(""); setOwnerLocation(""); setOwnerDateOfBirth(""); setOwnerAllowMultiple(false); setOwnerMaxBusinesses("1");
    }
  ...
  function closeOwnerModal() {
    ...
    setTimeout(() => { setOwnerName(""); setOwnerNationalId(""); setOwnerLocation(""); setOwnerDateOfBirth(""); setOwnerAllowMultiple(false); setOwnerMaxBusinesses("1"); }, 300);
  }

  function handleSaveOwner() {
    if (!ownerName.trim()) { Alert.alert("Error", "Full name is required."); return; }
    const allowMultiple = ownerAllowMultiple ? 1 : 0;
    const maxBusinesses = parseInt(ownerMaxBusinesses) || 1;
    let result;
    if (editingOwner) result = updateBusinessOwner(editingOwner.id, ownerName, ownerNationalId || null, ownerLocation || null, ownerDateOfBirth || null, allowMultiple, maxBusinesses);
    else result = addBusinessOwner(ownerName, ownerNationalId || null, ownerLocation || null, ownerDateOfBirth || null, allowMultiple, maxBusinesses);
    if (result.success) { closeOwnerModal(); loadAllData(); }
    Alert.alert(result.success ? "Success" : "Error", result.message);
  }

  function handleDeleteOwner(id: number) {
    Alert.alert("Confirm Delete", "Delete this owner?", [
      { text: "Cancel", style: "cancel" },
      { text: "Delete", style: "destructive", onPress: () => { const result = deleteBusinessOwner(id); if (result.success) loadAllData(); Alert.alert(result.success ? "Success" : "Error", result.message); } },
    ]);
  }
  ...
  function openTypeModal(type?: BusinessType) {
    if (type) {
      ...
      setEditingType(null); setTypeName(""); setTypeDescription(""); setTypeAmountCharge("");
    }
  ...
  function closeTypeModal() {
    ...
    setTimeout(() => { setTypeName(""); setTypeDescription(""); setTypeAmountCharge(""); }, 300);
  }

  function handleSaveType() {
    if (!typeName.trim()) { Alert.alert("Error", "Type name is required."); return; }
    const amountCharge = parseFloat(typeAmountCharge) || 0;
    let result;
    if (editingType) result = updateBusinessType(editingType.id, typeName, typeDescription || null, amountCharge);
    else result = addBusinessType(typeName, typeDescription || null, amountCharge);
    if (result.success) { closeTypeModal(); loadAllData(); }
    Alert.alert(result.success ? "Success" : "Error", result.message);
  }

  function handleDeleteType(id: number) {
    Alert.alert("Confirm Delete", "Delete this type?", [
      { text: "Cancel", style: "cancel" },
      { text: "Delete", style: "destructive", onPress: () => { const result = deleteBusinessType(id); if (result.success) loadAllData(); Alert.alert(result.success ? "Success" : "Error", result.message); } },
    ]);
  }
  ...
  const renderBusinessForm = () => (
    <View>
      ...
      <View style={styles.whiteInputWrap}>
        ...
        <TextInput style={styles.whiteInput} placeholder="Business address" placeholderTextColor="#999999" value={address} onChangeText={setAddress} />
      </View>

      <Text style={styles.whiteModalLabel}>PHONE</Text>
      <View style={styles.whiteInputWrap}>
        <Ionicons name="call" size={16} color="#999999" style={styles.whiteInputIcon} />
        <TextInput style={styles.whiteInput} placeholder="Contact phone" placeholderTextColor="#999999" value={businessPhone} onChangeText={setBusinessPhone} keyboardType="phone-pad" />
      </View>

      <Text style={styles.whiteModalLabel}>EMAIL (OPTIONAL)</Text>
      <View style={styles.whiteInputWrap}>
        <Ionicons name="mail" size={16} color="#999999" style={styles.whiteInputIcon} />
        <TextInput style={styles.whiteInput} placeholder="business@email.com" placeholderTextColor="#999999" value={businessEmail} onChangeText={setBusinessEmail} keyboardType="email-address" autoCapitalize="none" />
      </View>

      {editingBusiness && (
        <View style={styles.whiteSwitchRow}>
          <Text style={styles.whiteModalLabel}>ACTIVE STATUS</Text>
          <Switch value={isActive} onValueChange={setIsActive} trackColor={{ false: "#e5e7eb", true: "rgba(92,140,232,0.5)" }} thumbColor={isActive ? "#5C8CE8" : "#9ca3af"} />
        </View>
      )}
  ...
  const renderOwnerForm = () => (
    <View>
      ...
      <View style={styles.whiteInputWrap}>
        ...
        <TextInput style={styles.whiteInput} placeholder="Enter full name" placeholderTextColor="#999999" value={ownerName} onChangeText={setOwnerName} autoCapitalize="words" />
      </View>

      <Text style={styles.whiteModalLabel}>NATIONAL ID</Text>
      <View style={styles.whiteInputWrap}>
        <Ionicons name="card" size={16} color="#999999" style={styles.whiteInputIcon} />
        <TextInput style={styles.whiteInput} placeholder="National ID number" placeholderTextColor="#999999" value={ownerNationalId} onChangeText={setOwnerNationalId} autoCapitalize="words" />
      </View>

      <Text style={styles.whiteModalLabel}>LOCATION</Text>
      <View style={styles.whiteInputWrap}>
        <Ionicons name="location" size={16} color="#999999" style={styles.whiteInputIcon} />
        <TextInput style={styles.whiteInput} placeholder="City / District / Area" placeholderTextColor="#999999" value={ownerLocation} onChangeText={setOwnerLocation} />
      </View>

      <Text style={styles.whiteModalLabel}>DATE OF BIRTH</Text>
      <TouchableOpacity style={styles.whiteInputWrap} onPress={() => { if (ownerDateOfBirth) { const parsed = new Date(ownerDateOfBirth); if (!isNaN(parsed.getTime())) setDate(parsed); } setShowCalendar(true); }} activeOpacity={0.8}>
        <Ionicons name="calendar" size={16} color="#999999" style={styles.whiteInputIcon} />
        <Text style={[styles.whiteInput, { color: ownerDateOfBirth ? '#111827' : '#999999' }]}>{ownerDateOfBirth || 'YYYY-MM-DD'}</Text>
      </TouchableOpacity>

      <View style={styles.whiteSwitchRow}>
        <Text style={styles.whiteModalLabel}>ALLOW MULTIPLE BUSINESSES</Text>
        <Switch value={ownerAllowMultiple} onValueChange={setOwnerAllowMultiple} trackColor={{ false: "#e5e7eb", true: "rgba(92,140,232,0.5)" }} thumbColor={ownerAllowMultiple ? "#5C8CE8" : "#9ca3af"} />
      </View>
      ...
      {ownerAllowMultiple && (
        <>
          ...
          <View style={styles.whiteInputWrap}>
            ...
            <TextInput style={styles.whiteInput} placeholder="Enter max number" placeholderTextColor="#999999" value={ownerMaxBusinesses} onChangeText={setOwnerMaxBusinesses} keyboardType="number-pad" />
          </View>
        </>
      )}
  ...
  const renderTypeForm = () => (
    <View>
      ...
      <View style={styles.whiteInputWrap}>
        ...
        <TextInput style={styles.whiteInput} placeholder="e.g. Retail Shop" placeholderTextColor="#999999" value={typeName} onChangeText={setTypeName} />
      </View>
      ...
      <View style={[styles.whiteInputWrap, { height: 100, alignItems: "flex-start", paddingTop: 12 }]}>
        ...
        <TextInput style={[styles.whiteInput, { height: 80, textAlignVertical: "top" }]} placeholder="Brief description of this business type" placeholderTextColor="#999999" value={typeDescription} onChangeText={setTypeDescription} multiline numberOfLines={3} />
      </View>
      ...
      <View style={styles.whiteInputWrap}>
        ...
        <TextInput style={styles.whiteInput} placeholder="0.00" placeholderTextColor="#999999" value={typeAmountCharge} onChangeText={setTypeAmountCharge} keyboardType="decimal-pad" />
      </View>
    ...
    <LinearGradient colors={["#456da5", "#073474", "#5C8CE8"]} style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
        ...
        <View style={styles.headerRight}>
          <TouchableOpacity style={styles.addHeaderBtn} onPress={() => { if (activeTab === "businesses") openBusinessModal(); else if (activeTab === "owners") openOwnerModal(); else openTypeModal(); }}>
            <Ionicons name="add-outline" size={22} color="#FFFFFF" />
          </TouchableOpacity>
        </View>
      </View>

      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="rgba(255,255,255,0.6)" colors={["#fbfcff"]} progressBackgroundColor="rgba(255,255,255,0.1)" />} contentContainerStyle={{ paddingBottom: insets.bottom + 120 }}>
        ...
        <View style={styles.searchWrap}>
          ...
          <TextInput style={styles.searchInput} placeholder={activeTab === "businesses" ? "Search businesses..." : activeTab === "owners" ? "Search owners..." : "Search types..."} placeholderTextColor="rgba(255,255,255,0.3)" value={activeTab === "businesses" ? businessSearchQuery : activeTab === "owners" ? ownerSearchQueryTab : typeSearchQuery} onChangeText={activeTab === "businesses" ? setBusinessSearchQuery : activeTab === "owners" ? setOwnerSearchQueryTab : setTypeSearchQuery} />
          {((activeTab === "businesses" && businessSearchQuery) || (activeTab === "owners" && ownerSearchQueryTab) || (activeTab === "types" && typeSearchQuery)) && (
            <TouchableOpacity onPress={() => { if (activeTab === "businesses") setBusinessSearchQuery(""); else if (activeTab === "owners") setOwnerSearchQueryTab(""); else setTypeSearchQuery(""); }}>
          ...
          <View style={styles.listSection}>
            {filteredBusinesses.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>{businessSearchQuery.trim() ? "No businesses match your search" : "No businesses registered yet"}</Text>
              </View>
            ) : (
                ...
                <View key={biz.id} style={styles.listCard}>
                  <View style={styles.listCardHeader}>
                    <View style={styles.listCardLeft}>
                      <View style={styles.bizIconWrap}><Ionicons name="business-outline" size={20} color="rgba(255,255,255,0.7)" /></View>
                      <View style={{ flex: 1 }}>
                        ...
                        {biz.owner_national_id && <Text style={styles.listCardSubMeta}>ID: {biz.owner_national_id}</Text>}
                      </View>
                    </View>
                    <View style={styles.listCardRight}>
                      <TouchableOpacity style={styles.moreBtn} onPress={() => setExpandedCardId(expandedCardId === biz.id ? null : biz.id)}>
                        <Ionicons name={expandedCardId === biz.id ? "ellipsis-horizontal" : "ellipsis-horizontal-outline"} size={20} color="rgba(255,255,255,0.5)" />
                      </TouchableOpacity>
                      <Switch value={biz.is_active === 1} onValueChange={() => { toggleBusinessStatus(biz.id, biz.is_active); loadAllData(); }} trackColor={{ false: "rgba(255,255,255,0.15)", true: "rgba(92,140,232,0.5)" }} thumbColor={biz.is_active === 1 ? "#5C8CE8" : "rgba(255,255,255,0.5)"} />
                    </View>
                  </View>
                  <View style={styles.listCardDetails}>
                    {biz.registration_number && <Text style={styles.detailText}>Reg: {biz.registration_number}</Text>}
                  ...
                  {expandedCardId === biz.id && (
                    <View style={styles.listCardActions}>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => { openBusinessModal(biz); setExpandedCardId(null); }}><Ionicons name="create-outline" size={16} color="rgba(255,255,255,0.6)" /><Text style={styles.actionBtnText}>Edit</Text></TouchableOpacity>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => { handleDeleteBusiness(biz.id); setExpandedCardId(null); }}><Ionicons name="trash-outline" size={16} color="rgba(255,100,100,0.8)" /><Text style={[styles.actionBtnText, { color: "rgba(255,100,100,0.8)" }]}>Delete</Text></TouchableOpacity>
                    </View>
                  )}
                </View>
              ))
          ...
          <View style={styles.listSection}>
            {filteredOwners.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>{ownerSearchQueryTab.trim() ? "No owners match your search" : "No business owners registered yet"}</Text>
              </View>
            ) : (
                  ...
                  <View style={styles.listCardHeader}>
                    <View style={styles.listCardLeft}>
                      <View style={[styles.bizIconWrap, { backgroundColor: "rgba(255,255,255,0.06)" }]}>
                        <Text style={styles.avatarInitial}>{owner.full_name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2)}</Text>
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={styles.listCardTitle}>{owner.full_name}</Text>
                        {owner.national_id && <Text style={styles.listCardMeta}>ID: {owner.national_id}</Text>}
                        {owner.location && <Text style={styles.listCardSubMeta}>Location: {owner.location}</Text>}
                        {owner.date_of_birth && <Text style={styles.listCardSubMeta}>DOB: {owner.date_of_birth}</Text>}
                        <Text style={styles.listCardSubMeta}>{owner.allow_multiple_businesses === 1 ? `Multi-Business: Yes (Max ${owner.max_businesses_count})` : "Multi-Business: No"}</Text>
                      </View>
                    </View>
                    <TouchableOpacity style={styles.moreBtn} onPress={() => setExpandedCardId(expandedCardId === owner.id ? null : owner.id)}>
                      <Ionicons name={expandedCardId === owner.id ? "ellipsis-horizontal" : "ellipsis-horizontal-outline"} size={20} color="rgba(255,255,255,0.5)" />
                    </TouchableOpacity>
                  </View>
                  {expandedCardId === owner.id && (
                    <View style={styles.listCardActions}>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => { openOwnerModal(owner); setExpandedCardId(null); }}><Ionicons name="create-outline" size={16} color="rgba(255,255,255,0.6)" /><Text style={styles.actionBtnText}>Edit</Text></TouchableOpacity>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => { handleDeleteOwner(owner.id); setExpandedCardId(null); }}><Ionicons name="trash-outline" size={16} color="rgba(255,100,100,0.8)" /><Text style={[styles.actionBtnText, { color: "rgba(255,100,100,0.8)" }]}>Delete</Text></TouchableOpacity>
                    </View>
                  )}
                </View>
              ))
          ...
          <View style={styles.listSection}>
            {filteredTypes.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>{typeSearchQuery.trim() ? "No types match your search" : "No business types defined yet"}</Text>
              </View>
            ) : (
              filteredTypes.map((type) => (
                <View key={type.id} style={styles.listCard}>
                  <View style={styles.listCardHeader}>
                    ...
                    <TouchableOpacity style={styles.moreBtn} onPress={() => setExpandedCardId(expandedCardId === type.id ? null : type.id)}>
                      <Ionicons name={expandedCardId === type.id ? "ellipsis-horizontal" : "ellipsis-horizontal-outline"} size={20} color="rgba(255,255,255,0.5)" />
                    </TouchableOpacity>
                  </View>
                  {expandedCardId === type.id && (
                    <View style={styles.listCardActions}>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => { openTypeModal(type); setExpandedCardId(null); }}><Ionicons name="create-outline" size={16} color="rgba(255,255,255,0.6)" /><Text style={styles.actionBtnText}>Edit</Text></TouchableOpacity>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => { handleDeleteType(type.id); setExpandedCardId(null); }}><Ionicons name="trash-outline" size={16} color="rgba(255,100,100,0.8)" /><Text style={[styles.actionBtnText, { color: "rgba(255,100,100,0.8)" }]}>Delete</Text></TouchableOpacity>
                    </View>
                  )}
                </View>
              ))
      ...
      <Modal visible={ownerModalVisible} animationType="slide" transparent onRequestClose={closeOwnerModal}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.whiteModalOverlay}>
          ...
          <View style={styles.whiteModalCard}>
            <View style={styles.whiteModalHeader}>
              <Text style={styles.whiteModalTitle}>{editingOwner ? "Edit Owner" : "Add Business Owner"}</Text>
              <TouchableOpacity onPress={closeOwnerModal} style={styles.whiteModalClose}><Ionicons name="close" size={22} color="#666666" /></TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.whiteModalScrollContent}>
              {renderOwnerForm()}
            </ScrollView>
            <View style={styles.whiteModalActions}>
              <TouchableOpacity style={styles.whiteCancelBtn} onPress={closeOwnerModal} activeOpacity={0.7}><Text style={styles.whiteCancelText}>Cancel</Text></TouchableOpacity>
      ...
      <Modal visible={typeModalVisible} animationType="slide" transparent onRequestClose={closeTypeModal}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.whiteModalOverlay}>
          ...
          <View style={styles.whiteModalCard}>
            <View style={styles.whiteModalHeader}>
              <Text style={styles.whiteModalTitle}>{editingType ? "Edit Type" : "Add Business Type"}</Text>
              <TouchableOpacity onPress={closeTypeModal} style={styles.whiteModalClose}><Ionicons name="close" size={22} color="#666666" /></TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.whiteModalScrollContent}>
              {renderTypeForm()}
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: "#FFFFFF", fontSize: 28, fontWeight: "800", letterSpacing: 0.3 },
  headerSubtitle: { color: "rgba(255,255,255,0.45)", fontSize: 13, fontWeight: "500", marginTop: 4 },
  addHeaderBtn: { width: 40, height: 40, borderRadius: 12, backgroundColor: "rgba(255, 255, 255, 0.23)", justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: "rgba(255, 255, 255, 0.23)" },
  tabBar: { flexDirection: "row", gap: 8, paddingHorizontal: 20, marginBottom: 12 },
  tabBtn: { flex: 1, paddingVertical: 10, borderRadius: 12, backgroundColor: "rgba(255, 255, 255, 0.23)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", alignItems: "center" },
  tabBtnActive: { backgroundColor: "rgba(92,140,232,0.25)", borderColor: "rgba(92,140,232,0.5)" },
  tabText: { color: "rgba(255,255,255,0.5)", fontSize: 13, fontWeight: "600" },
  tabTextActive: { color: "#FFFFFF" },
  searchWrap: { flexDirection: "row", alignItems: "center", backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.23)", borderRadius: 14, paddingHorizontal: 14, height: 46, marginHorizontal: 20, marginBottom: 12 },
  searchIcon: { marginRight: 10 },
  searchInput: { flex: 1, color: "#FFFFFF", fontSize: 14, fontWeight: "500" },
  listSection: { paddingHorizontal: 20, gap: 10 },
  listCard: { backgroundColor: "rgba(255, 255, 255, 0.23)", borderRadius: 16, padding: 16, borderWidth: 1, borderColor: "rgba(255,255,255,0.23)" },
  listCardHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" },
  listCardLeft: { flexDirection: "row", alignItems: "flex-start", gap: 12, flex: 1 },
  listCardRight: { flexDirection: "row", alignItems: "center", gap: 8 },
  moreBtn: { width: 32, height: 32, borderRadius: 8, backgroundColor: "rgba(255,255,255,0.06)", justifyContent: "center", alignItems: "center" },
  bizIconWrap: { width: 40, height: 40, borderRadius: 12, backgroundColor: "rgba(255,255,255,0.08)", justifyContent: "center", alignItems: "center" },
  avatarInitial: { color: "#FFFFFF", fontSize: 14, fontWeight: "700" },
  listCardTitle: { color: "#FFFFFF", fontSize: 15, fontWeight: "600" },
  listCardMeta: { color: "hsl(0, 0%, 100%)", fontSize: 12, fontWeight: "400", marginTop: 2 },
  listCardSubMeta: { color: "hsla(0, 0%, 100%, 0.99)", fontSize: 11, fontWeight: "400", marginTop: 1 },
  listCardDetails: { marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: "rgb(255, 255, 255)", gap: 4 },
  detailText: { color: "rgb(255, 255, 255)", fontSize: 12, fontWeight: "400" },
  listCardActions: { flexDirection: "row", gap: 16, marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.06)" },
  actionBtn: { flexDirection: "row", alignItems: "center", gap: 6 },
  actionBtnText: { color: "rgba(255,255,255,0.6)", fontSize: 13, fontWeight: "500" },
  ...
  selectedOwnerSub: { color: "#9ca3af", fontSize: 13, fontWeight: "500" },
  whiteSwitchRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 14 },
  whiteModalActions: { flexDirection: "row", gap: 12, marginTop: 8, paddingTop: 12, borderTopWidth: 1, borderTopColor: "#f3f4f6" },
