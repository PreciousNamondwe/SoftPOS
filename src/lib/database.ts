...
export function initializeDatabase(): void {
    // ✅ Use execSync with all statements together instead of withTransactionSync
    const allSchema = `
    ...
    console.log("✅ Database initialized with roles.");
}
// ============================================================
// ROLES CRUD
// ============================================================
...
export function getRoleById(id: number): Role | null {
    return db.getFirstSync<Role>("SELECT * FROM roles WHERE id = ?;", [id]);
}

/** Get a single role by its natural key (role_key) */
export function getRoleByKey(roleKey: string): Role | null {
    return db.getFirstSync<Role>("SELECT * FROM roles WHERE role_key = ?;", [roleKey]);
}

/** Create a new role */
export function createRole(roleKey: string, roleLabel: string, description?: string, color?: string): number {
    const result = db.runSync(
        `INSERT INTO roles (role_key, role_label, description, color, is_synced) VALUES (?, ?, ?, ?, 0);`,
...
export function updateRole(id: number, updates: Partial<Pick<Role, "role_label" | "description" | "color">>): void {
    ...
    if (updates.role_label !== undefined) {
        ...
        values.push(updates.role_label);
    }
...
/** Delete a role (fails if any user references it due to FK RESTRICT) */
...
/** Get user by user_id (including inactive) with role details */
...
/** Create a new user (pinHash must be pre-hashed) */
...
export function updateUser(userId: string, updates: Partial<Pick<UserRecord, "full_name" | "role" | "is_active">>): void {
    ...
    values.push(userId);
    db.runSync(`UPDATE user SET ${fields.join(", ")} WHERE user_id = ?;`, values);
}

/** Update user's PIN hash */
export function updateUserPin(userId: string, newPinHash: string): void {
    db.runSync(
        "UPDATE user SET pin_hash = ?, is_synced = 0, updated_at = datetime('now') WHERE user_id = ?;",
        [newPinHash, userId]
    );
}
...
export function toggleUserStatus(userId: string): number {
    db.runSync(
        "UPDATE user SET is_active = CASE WHEN is_active = 1 THEN 0 ELSE 1 END, is_synced = 0, updated_at = datetime('now') WHERE user_id = ?;",
        [userId]
    );
...
export function searchUsers(query: string): UserWithRole[] {
    ...
    return db.getAllSync<UserWithRole>(`
        ...
        FROM user u
        LEFT JOIN roles r ON u.role = r.role_key
        WHERE LOWER(u.user_id) LIKE ? OR LOWER(u.full_name) LIKE ?
        ORDER BY u.created_at DESC;
    `, [likeQuery, likeQuery]);
}
...
export function updateLastLogin(userId: string): void {
    db.runSync(
        "UPDATE user SET last_login = datetime('now'), is_synced = 0, updated_at = datetime('now') WHERE user_id = ?;",
        [userId]
    );
}
...
export function createSession(userId: string, jwtToken: string | null, expiresAt: string, isOffline: boolean = false): number {
    const result = db.runSync(
        "INSERT INTO sessions (user_id, jwt_token, expires_at, is_offline, is_synced) VALUES (?, ?, ?, ?, 0);",
...
export function logAudit(userId: string, action: string, details?: object): void {
    db.runSync(
        "INSERT INTO audit_logs (user_id, action, details, is_synced) VALUES (?, ?, ?, 0);",
